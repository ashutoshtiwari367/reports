...
$stats = [
    ...
    'total_collection' => $pdo->query("SELECT COALESCE(SUM(p.amount),0) FROM emi_payments p JOIN emi_schedule e ON p.emi_id=e.id JOIN loans l ON e.loan_id=l.id WHERE l.status != 'cancelled' $shopJoinFilter")->fetchColumn(),
    'total_emi'        => $pdo->query("SELECT COALESCE(SUM(e.emi_amount),0) FROM emi_schedule e JOIN loans l ON e.loan_id=l.id WHERE l.status != 'cancelled' $shopJoinFilter")->fetchColumn(),
    'total_pending'    => $pdo->query("SELECT COALESCE(SUM(e.emi_amount - e.paid_amount),0) FROM emi_schedule e JOIN loans l ON e.loan_id=l.id WHERE e.status IN ('due','partial','overdue') AND l.status != 'cancelled' $shopJoinFilter")->fetchColumn(),
    'closed_loans'     => $pdo->query("SELECT COUNT(*) FROM loans WHERE status='closed' $shopFilter")->fetchColumn(),
    'expected_profit'  => getShopTotalProfit(isShopAdmin() ? getShopId() : null),
    'received_profit'  => getShopReceivedProfit(isShopAdmin() ? getShopId() : null),
    'total_cost'       => getShopTotalLagat(isShopAdmin() ? getShopId() : null),
];
